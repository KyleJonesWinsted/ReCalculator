    ...
    var body: some View {
        LazyVGrid(columns: columns) {
            ...
            ForEach(0..<labels.count, id: \.self) { idx in
                ...
                DigitButton(label: digit, color: colors[digit] ?? .buttonGray) {
                    input.append(digit)
                }
                .gridInfoId(idx)
            }
...
struct AddVariableButton: View {
    var width: CGFloat
    var addVariable: () -> Void
    @Environment(\.isZoomed) private var isZoomed
    ...
    var body: some View {
        Button {
            ...
            Text("+ Add Variable")
                .font(isZoomed ? .headline : .title2)
                .frame(width: width, height: isZoomed ? 35 : 53)
        }
        .frame(width: width)
...
struct DigitButton: View {
    ...
    var body: some View {
        Button {
            ...
            Text(label)
                .font(isZoomed ? .headline : .largeTitle)
                .frame(maxWidth: .infinity)
                .padding(7)
        }
